      ...
      Future.microtask(()async{
        ...
        if(profile != null && profile.isNotEmpty) {
          ...
          ref
              ...
              .state = landlordDetails.value['Location'] ?? 'Kakamega Town';
                    ...
                    children: [
                      GestureDetector(
                        onTap: ()async{
                          ...
                          //TODO: add image picking logic for adding profile photo
                      ...
                      GestureDetector(
                        onTap: () async{
                          try {
                            await ref.watch(firebaseServices)
                                .sendEmailVerification();
                            if(!context.mounted) return;
                            SnackBars.showInfoSnackBar(context, 'An email verification link has been sent to your email. Click the link to be verified');
                          }catch(e){
                            logger.e(e);
                            SnackBars.showErrorSnackBar(context, 'An error occurred $e');
                          }
                        },
                        child: Container(
                          width: width,
                          ...
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Center(child: Text('Verify Now')),
                          ),
                        ),
                      ),
                      FunctionButton(text: 'Update Profile', onPressed: () async{
                        if(formKey.currentState!.validate()) {
                          try{
                          await ref.watch(firebaseServices).createLandlordProfile(ref, nameController.text, emailController.text, phoneController.text, ref.watch(houseLocationProvider), null);
                          if(!context.mounted) return;
                          SnackBars.showSuccessSnackBar(context, 'Profile Updated successfully');
                        }
                        catch(e){
                          logger.e(e);
                          SnackBars.showErrorSnackBar(context, 'An error occurred trying to update your profile. Please try again');
                        }
                        }
